...
export function Portfolio({ data }) {
  ...
  const tabs = [{ link: "all" }, { link: "ui" }, { link: "code" }];
      ...
      <ul className="portfolio__tabs">
        {tabs.map((tab) => (
          <li
            key={tab.link}
            onClick={() => {
              setActivelink(tab.link);
            }}
            className={
              activelink === tab.link
                ? "portfolio__link portfolio__link_active"
                : "portfolio__link"
            }
          >
            <span data-testid={tab.link}>{tab.link}</span>
          </li>
        ))}
      </ul>
        ...
        {filterWorks.map((work) => (
          <li key={work.id} className={`portfolio__item ${work.type}`}>
            ...
            <div className="portfolio__info">
              <h3 className="portfolio__title">{work.title}</h3>
              <Info data={work.text} />
              <a className="portfolio__sourse" href="http//github.com">
                {" "}
                View source
              </a>
            </div>
